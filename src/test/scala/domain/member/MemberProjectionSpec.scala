...
class MemberProjectionSpec extends FunSuite:
  ...
  val projection: MemberProjection =
    MemberProjection.make(MemberRepository.make(dummyPool))
  ...
  test("filter includes only MemberRegistered"):
    ...
    assert(filter.contains(EventTypeName.of[MemberRegistered]))
    assertEquals(filter.size, 1)

  test("resolveKeys extracts memberId from MemberRegistered"):
    val memberId = UUID.randomUUID()
    val keys = projection.resolveKeys(
      envelope(MemberRegistered(memberId, "Alice", LocalDate.of(1990, 1, 1)))
    )
  ...
  test("handle creates a new Member from None + MemberRegistered"):
    ...
    assertEquals(
      result,
      Some(Member(memberId, "Alice", LocalDate.of(1990, 1, 1)))
    )
